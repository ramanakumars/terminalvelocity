...
for species in spec_data.keys():
    ...
    for planet in planet_data.keys():
        ...
        for phase in ["rain","ice","snow"]:
            ...
            for p in Prange:
                if(p != Pref):
                    gammai = np.log(vT[species][planet][phase][p]/vT[species][planet][phase][Pref])/np.log(Pref/p)
                    gamma_p = np.average(gammai)
                    # print(gammai.mean(), gammai.std())
            ...
            # plt.plot(xData, zData, '--', color='grey', linewidth=0.5)
            # plt.plot(xData, 10.**par[0]*(xData)**(par[1]), 'k-')
            # plt.show()
